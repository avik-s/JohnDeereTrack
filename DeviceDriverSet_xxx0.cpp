 ...
 * @LastEditTime: 2021-01-06 13:57:44
...
void DeviceDriverSet_RBGLED::DeviceDriverSet_RBGLED_xxx(
    uint16_t Duration, uint8_t Traversal_Number, CRGB colour) {
...
void DeviceDriverSet_RBGLED::DeviceDriverSet_RBGLED_Init(
...
void DeviceDriverSet_RBGLED::DeviceDriverSet_RBGLED_Test(void) {
  ...
  FastLED.show();
  delay(50);
  DeviceDriverSet_RBGLED_xxx(50 /*Duration*/, 5 /*Traversal_Number*/,
                             CRGB::Black);
}
#endif

void DeviceDriverSet_RBGLED::DeviceDriverSet_RBGLED_Color(uint8_t LED_s,
                                                          uint8_t r, uint8_t g,
                                                          uint8_t b) {
  if (LED_s > NUM_LEDS)
    return;
...
void DeviceDriverSet_passiveBuzzer::DeviceDriverSet_passiveBuzzer_Test(void) {
  for (int thisNote = 0; thisNote < 8; thisNote++) {
    tone(5, melody[thisNote],
         duration); // pinxx output the voice, every scale is 0.5 sencond
    delay(1000);    // Output the voice after several minutes
  }
}
#endif

void DeviceDriverSet_passiveBuzzer::
    DeviceDriverSet_passiveBuzzer_controlMonosyllabic(
        uint8_t controlMonosyllabic, uint32_t Duration) {
  if (controlMonosyllabic > 7) // 0----7
    controlMonosyllabic = 7;
  // Serial.println(Duration);
  tone(5, melody[controlMonosyllabic],
       Duration); // pinxx output the voice, every scale is 0.5 sencond
  delay(Duration);
}

void DeviceDriverSet_passiveBuzzer::DeviceDriverSet_passiveBuzzer_controlAudio(
    uint16_t controlAudio, uint32_t Duration) {
...
void DeviceDriverSet_passiveBuzzer::DeviceDriverSet_passiveBuzzer_Scale_c8(
    uint32_t Duration) {
...
void DeviceDriverSet_Key::DeviceDriverSet_Key_Init(void) {
  ...
  attachPCINT(digitalPinToPCINT(PIN_Key), attachPinChangeInterrupt_GetKeyValue,
              FALLING);
}
...
float DeviceDriverSet_ITR20001::DeviceDriverSet_ITR20001_getAnaloguexxx_L(
    void) {
  return analogRead(PIN_ITR20001xxxL);
}
float DeviceDriverSet_ITR20001::DeviceDriverSet_ITR20001_getAnaloguexxx_R(
    void) {
...
void DeviceDriverSet_Motor::DeviceDriverSet_Motor_Test(void) {
  ...
  analogWrite(PIN_Motor_PWMB, 255); // Z
}
#endif
...
void DeviceDriverSet_Motor::DeviceDriverSet_Motor_control(
    boolean direction_A, uint8_t speed_A, // A组电机参数
    boolean direction_B, uint8_t speed_B, // B组电机参数
    boolean controlED                     // AB使能允许 true
    )                                     // 电机控制
{
  // if (speed_A > speed_Max || speed_B > speed_Max)
  // //大于最大输出速度量视为不合法值
  //   return;
...
void DeviceDriverSet_ULTRASONIC::DeviceDriverSet_ULTRASONIC_Get(
...
void DeviceDriverSet_STM8S003F3_IR::DeviceDriverSet_STM8S003F3_IR_Get(
    uint16_t *STM8S003F3_IRL /*out*/, uint16_t *STM8S003F3_IRM /*out*/,
    uint16_t *STM8S003F3_IRR /*out*/) {
  uint8_t STM8S003F3_IR_IIC_buff[8];
  uint8_t a = 0;
  Wire.requestFrom(STM8S003F3_IR_devAddr,
                   8);     // request 6 bytes from slave device #2
  while (Wire.available()) // slave may send less than requested
  {
    STM8S003F3_IR_IIC_buff[a++] = Wire.read(); // receive a byte as character
  }

  if ((STM8S003F3_IR_IIC_buff[0] == 0XA0) &&
      (STM8S003F3_IR_IIC_buff[7] == 0XB0)) {

    *STM8S003F3_IRR =
        (STM8S003F3_IR_IIC_buff[1] << 8) | (STM8S003F3_IR_IIC_buff[2]);
    *STM8S003F3_IRL =
        (STM8S003F3_IR_IIC_buff[3] << 8) | (STM8S003F3_IR_IIC_buff[4]);
    *STM8S003F3_IRM =
        (STM8S003F3_IR_IIC_buff[5] << 8) | (STM8S003F3_IR_IIC_buff[6]);
  } else {
    /* code */
    // Serial.println("Contact Changhua :STM8S003F3_IR data error"); // print
    // the character
    return;
  }
}
...
void DeviceDriverSet_STM8S003F3_IR::DeviceDriverSet_STM8S003F3_IR_Test(void) {
  ...
  if ((STM8S003F3_IR_IIC_buff[0] == 0XA0) &&
    ...
    Serial.print("\t   L:");
    Serial.print((STM8S003F3_IR_IIC_buff[3] << 8) |
                 (STM8S003F3_IR_IIC_buff[4]));

    Serial.print("\t   M:");
    Serial.print((STM8S003F3_IR_IIC_buff[5] << 8) |
                 (STM8S003F3_IR_IIC_buff[6]));

    Serial.print("\t   R:");
    Serial.println((STM8S003F3_IR_IIC_buff[1] << 8) |
                   (STM8S003F3_IR_IIC_buff[2]));
...
void DeviceDriverSet_MPU6050::DeviceDriverSet_MPU6050_dveGetEulerAngles(
    float *is_yaw) {
  // uint8_t STM8S003F3_MPU6050_IIC_buff[6];
  // uint8_t a = 0;
  // Wire.requestFrom(STM8S003F3_MPU6050_devAddr, 6); // request 6 bytes from
  // slave device #2 while (Wire.available())                         // slave
  // may send less than requested
  // {
  //   STM8S003F3_MPU6050_IIC_buff[a++] = Wire.read(); // receive a byte as
  //   character
  // }

  // if ((STM8S003F3_MPU6050_IIC_buff[0] == 0XA1) &&
  // (STM8S003F3_MPU6050_IIC_buff[5] == 0XB1))
  // {
  //   //*gyro = ((STM8S003F3_MPU6050_IIC_buff[1] << 8) |
  //   (STM8S003F3_MPU6050_IIC_buff[2])) / 1000.00; *is_yaw =
  //   ((STM8S003F3_MPU6050_IIC_buff[3] << 8) |
  //   (STM8S003F3_MPU6050_IIC_buff[4])) / 100.00;
  // }
  // else
  // {
  //   /* code */
  //   //Serial.println("Contact Changhua :STM8S003F3_MPU6050 data error"); //
  //   print the character return;
  // }
  float gyro, yaw;
  DeviceDriverSet_MPU6050::DeviceDriverSet_MPU6050_dveGetEulerAngles(&gyro,
                                                                     &yaw);
  *is_yaw = yaw;
}

void DeviceDriverSet_MPU6050::DeviceDriverSet_MPU6050_dveGetEulerAngles(
    float *gyro, float *is_yaw) {
  uint8_t STM8S003F3_MPU6050_IIC_buff[6];
  uint8_t a = 0;
  Wire.requestFrom(STM8S003F3_MPU6050_devAddr,
                   6);     // request 6 bytes from slave device #2
  while (Wire.available()) // slave may send less than requested
  {
    STM8S003F3_MPU6050_IIC_buff[a++] =
  ...
  if ((STM8S003F3_MPU6050_IIC_buff[0] == 0XA1) &&
    ...
    *gyro = ((STM8S003F3_MPU6050_IIC_buff[1] << 8) |
            ...
            1000.00;
    *is_yaw = ((STM8S003F3_MPU6050_IIC_buff[3] << 8) |
               (STM8S003F3_MPU6050_IIC_buff[4])) /
              100.00;
  } else {
    /* code */
    // Serial.println("Contact Changhua :STM8S003F3_MPU6050 data error"); //
...
void DeviceDriverSet_MPU6050::DeviceDriverSet_MPU6050_Test(void) {
  ...
  uint8_t a = 0;
  Wire.requestFrom(STM8S003F3_MPU6050_devAddr,
                   4);     // request 6 bytes from slave device #2
  while (Wire.available()) // slave may send less than requested
  {
    STM8S003F3_MPU6050_IIC_buff[a++] =
        Wire.read(); // receive a byte as character
  ...
  if ((STM8S003F3_MPU6050_IIC_buff[0] == 0XA1) &&
      (STM8S003F3_MPU6050_IIC_buff[3] == 0XB1)) {
    Serial.print("STM8S003F3_MPU6050->is_yaw:      ");
    Serial.println((STM8S003F3_MPU6050_IIC_buff[1] << 8) |
                   (STM8S003F3_MPU6050_IIC_buff[2]));
    Serial.println(
        "...................................."); // print the character
  } else {
