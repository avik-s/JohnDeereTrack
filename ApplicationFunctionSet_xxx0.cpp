...
#define _is_print 1
#define _Test_print                                                            \
  0 // When testing, remember to set 0 after using the test to save controller
    // resources and load.
...
static void ApplicationFunctionSet_MetalCarLinearMotionControl(
    OwlCarMotionControl direction, uint8_t directionRecord, uint8_t speed,
    uint8_t Kp, uint8_t UpperLimit);
static void
ApplicationFunctionSet_OwlCarMotionControl(OwlCarMotionControl direction,
                                           uint8_t speed);
...
static bool ApplicationFunctionSet_OwlCarLeaveTheGround(void) {
  AppSTM8S003F3_IR.DeviceDriverSet_STM8S003F3_IR_Get(
      &Application_FunctionSet.TrackingData_L /*uint16_t * STM8S003F3_IRL out*/,
      &Application_FunctionSet.TrackingData_M /*uint16_t * STM8S003F3_IRM out*/,
      &Application_FunctionSet
           .TrackingData_R /*uint16_t * STM8S003F3_IRR out*/);
  /*R循迹状态更新*/
  Application_FunctionSet.TrackingDetectionStatus_R =
      function_xxx(Application_FunctionSet.TrackingData_R,
                   Application_FunctionSet.TrackingDetection_S,
                   Application_FunctionSet.TrackingDetection_E);
  /*M循迹状态更新*/
  Application_FunctionSet.TrackingDetectionStatus_M =
      function_xxx(Application_FunctionSet.TrackingData_M,
                   Application_FunctionSet.TrackingDetection_S,
                   Application_FunctionSet.TrackingDetection_E);
  /*L循迹状态更新*/
  Application_FunctionSet.TrackingDetectionStatus_L =
      function_xxx(Application_FunctionSet.TrackingData_L,
                   Application_FunctionSet.TrackingDetection_S,
                   Application_FunctionSet.TrackingDetection_E);
  // 检测小车是否离开地面
  if (Application_FunctionSet.TrackingData_L >
          Application_FunctionSet.TrackingDetection_CarLeaveTheGround &&
      Application_FunctionSet.TrackingData_M >
          Application_FunctionSet.TrackingDetection_CarLeaveTheGround &&
      Application_FunctionSet.TrackingData_R >
          Application_FunctionSet.TrackingDetection_CarLeaveTheGround) {
    Application_FunctionSet.Car_LeaveTheGround =
        true; // CHANGED FROM FALSE: Disable pickup detection
  } else {
...
static void ApplicationFunctionSet_MetalCarLinearMotionControl(
  ...
  if (en != directionRecord || millis() - is_time > 2) {
    ...
    // /*speed_A*/ 0, /*direction_B*/ direction_void, /*speed_B*/ 0,
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   static void
ApplicationFunctionSet_OwlCarMotionControl(OwlCarMotionControl direction,
uint8_t speed)
@ Functional function:  Owl Car 运动状态控制
@ Input parameters:     1# direction:前行（1）、后退（2）、
左前（3）、右前（4）、后左（5）、后右（6）、停止（6） 2# speed速度(0--255)
@ Output parameters:    none
@ Other Notes:
此为ApplicationFunctionSet层静态方法<调用硬件驱动层DeviceDriverSet_xxx0
Motor_control接口实现> $ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
static void
ApplicationFunctionSet_OwlCarMotionControl(OwlCarMotionControl direction,
  ...
  case /* constant-expression */
    ...
    if (Application_OwlCarxxx0.Functional_Mode == TraceBased_mode ||
        Application_OwlCarxxx0.Functional_Mode == CMD_TrajectoryControl) {
      AppMotor.DeviceDriverSet_Motor_control(
          /*direction_A*/ direction_just, /*speed_A*/ speed,
      ...
      //                                        /*speed_B*/ 0, /*controlED*/
      //                                        control_enable); //Motor control
      ApplicationFunctionSet_MetalCarLinearMotionControl(
          Forward, directionRecord, speed, Kp, UpperLimit);
      directionRecord = 1;
    }
  ...
  case /* constant-expression */ Backward:
    ...
    {
      AppMotor.DeviceDriverSet_Motor_control(
          /*direction_A*/ direction_back, /*speed_A*/ speed,
      ...
      //                                        control_enable); //Motor control
      ApplicationFunctionSet_MetalCarLinearMotionControl(
          Backward, directionRecord, speed, Kp, UpperLimit);
      directionRecord = 2;
    }
  ...
  case /* constant-expression */ LeftForward:
    ...
    AppMotor.DeviceDriverSet_Motor_control(
        /*direction_A*/ direction_just, /*speed_A*/ speed,
        /*direction_B*/ direction_just, /*speed_B*/ speed - 130,
        /*controlED*/ control_enable); // Motor control
  ...
  case /* constant-expression */ LeftBackward:
    ...
    AppMotor.DeviceDriverSet_Motor_control(
        /*direction_A*/ direction_back, /*speed_A*/ speed,
        /*direction_B*/ direction_back, /*speed_B*/ speed - 130,
        /*controlED*/ control_enable); // Motor control
    break;
  case /* constant-expression */ RightForward:
    ...
    directionRecord = 7;
    AppMotor.DeviceDriverSet_Motor_control(
        /*direction_A*/ direction_just, /*speed_A*/ speed - 130,
        /*direction_B*/ direction_just, /*speed_B*/ speed,
        /*controlED*/ control_enable); // Motor control
    break;
  case /* constant-expression */ RightBackward:
    ...
    directionRecord = 8;
    AppMotor.DeviceDriverSet_Motor_control(
        /*direction_A*/ direction_back, /*speed_A*/ speed - 130,
        /*direction_B*/ direction_back, /*speed_B*/ speed,
        /*controlED*/ control_enable); // Motor control
    break;
...
ApplicationFunctionSet::ApplicationFunctionSet_SensorDataUpdate(void)
@ Functional function:  Owl Car 传感器数据更新
@ Input parameters:     none
@ Output parameters:    none
@ Other Notes:
此为ApplicationFunctionSet层公共方法<调用硬件驱动层DeviceDriverSet_xxx0> $
...
void ApplicationFunctionSet::ApplicationFunctionSet_SensorDataUpdate(void) {
  ...
  { /*超声波数据更新：作用于避障*/
    ...
    UltrasoundDetectionStatus =
        function_xxx(UltrasoundData_cm, 0, ObstacleDetection); // 避障状态
  }
  {
  ...
  { /*电压状态更新*/
    ...
    if (millis() - VoltageData_time > 10) // 10ms 采集并更新一次
    {
      ...
      // Disable Voltage Shutdown Logic by hardcoding to false
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::ApplicationFunctionSet_Bootup(void)
@ Functional function:  Owl Car 开机动作设置
@ Input parameters:     none
@ Output parameters:    none
@ Other Notes:
此为ApplicationFunctionSet层公共方法<调用硬件驱动层<DeviceDriverSet_xxx0_H
/TM1640.h> 1# 表情面板显示 2# 有源蜂鸣器播放  C调音阶（播发八个音名） 3# Owl Car
进入空闲状态 $ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::ApplicationFunctionSet_RGB(void)
@ Functional function:  Owl Car RBG_LED 显示集合
@ Input parameters:     none
@ Output parameters:    none
@ Other Notes:
此为ApplicationFunctionSet层公共方法<调用硬件驱动层DeviceDriverSet_xxx0_H> 1#
Act on 低电压状态 2# Act on 模式控制 This is a set of RGB display logic control
sets $ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
void ApplicationFunctionSet::ApplicationFunctionSet_RGB(void) {
  ...
  if (true == VoltageDetectionStatus) // 1# Act on 低电压状态？
  {
    ...
    if ((millis() - getAnalogue_time) < 50) {
      AppRBG_LED.DeviceDriverSet_RBGLED_xxx(0 /*Duration*/,
                                            5 /*Traversal_Number*/, CRGB::Red);
    } else if ((millis() - getAnalogue_time) < 100) {
      AppRBG_LED.DeviceDriverSet_RBGLED_xxx(
          0 /*Duration*/, 5 /*Traversal_Number*/, CRGB::Black);
      getAnalogue_time = millis();
    }
  } else {
    ED = false;
    ...
    case /* constant-expression */ Standby_mode:
      /* code */
      {
        ...
        if (VoltageDetectionStatus == true) {
          AppRBG_LED.DeviceDriverSet_RBGLED_xxx(
              0 /*Duration*/, 5 /*Traversal_Number*/, CRGB::Red);
          delay(30);
          AppRBG_LED.DeviceDriverSet_RBGLED_xxx(
              0 /*Duration*/, 5 /*Traversal_Number*/, CRGB::Black);
          delay(30);
    ...
    case /* constant-expression */ TraceBased_mode:
      /* code */
      {
        FastLED_clear = 0xff;
        AppRBG_LED.DeviceDriverSet_RBGLED_xxx(
            0 /*Duration*/, 5 /*Traversal_Number*/, CRGB::Green);
      }
      break;
    case /* constant-expression */ ObstacleAvoidance_mode:
      /* code */
      {
        FastLED_clear = 0xff;
        AppRBG_LED.DeviceDriverSet_RBGLED_xxx(
            0 /*Duration*/, 5 /*Traversal_Number*/, CRGB::Yellow);
      }
      break;
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::ApplicationFunctionSet_Expression(void)
...
void ApplicationFunctionSet::ApplicationFunctionSet_Expression(void) {
  ...
  {
  case /* constant-expression */ Standby_mode: // 空闲模式时面板将以 1000ms
                                               // 的间隔循环显示表情logo集
    /* code */
    {
      ...
      if (millis() - Standby_mode_time > 1000) {
        ...
        Apptm1640.TM1640_FullScreenDisaple_led16x8(
            Apptm1640.Display_Model[Standby_mode_Panel] /*in*/);
  ...
  case /* constant-expression */ TraceBased_mode:
    /* code */
    {
      if (true == TrackingDetectionStatus_R &&
          true == TrackingDetectionStatus_L) {
        Apptm1640.TM1640_FullScreenDisaple_led16x8(
            Apptm1640.Display_Model[8] /*in*/);
      } else {
        Apptm1640.TM1640_FullScreenDisaple_led16x8(
            Apptm1640.Display_Model[2] /*in*/);
      }
    }
    break;
  case /* constant-expression */ ObstacleAvoidance_mode:
    /* code */
    {
      if (true == UltrasoundDetectionStatus) {
        Apptm1640.TM1640_FullScreenDisaple_led16x8(
            Apptm1640.Display_Model[0] /*in*/);
      } else {
        Apptm1640.TM1640_FullScreenDisaple_led16x8(
            Apptm1640.Display_Model[4] /*in*/);
      }
    }
    break;
  case /* constant-expression */ Rocker_mode:
      ...
      case /* constant-expression */ stop_it:
        /* code */
        {
          ...
          if (millis() - stop_it_time > 1000) {
            stop_it_time = millis();
            Apptm1640.TM1640_FullScreenDisaple_led16x8(
                Apptm1640.Display_Model[stop_it_Panel] /*in*/);
            stop_it_Panel += 1;
            if (stop_it_Panel > 9)
              stop_it_Panel = 0;
          }
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::ApplicationFunctionSet_Rocker(void)
@ Functional function:  Owl Car 摇杆
@ Input parameters:     none
@ Output parameters:    none
@ Other Notes:
此为ApplicationFunctionSet层公共方法<调用ApplicationFunctionSet_OwlCarMotionControl()>
...
void ApplicationFunctionSet::ApplicationFunctionSet_Rocker(void) {
  ...
  if (Application_OwlCarxxx0.Functional_Mode == Rocker_mode) {
    ApplicationFunctionSet_OwlCarMotionControl(
        Application_OwlCarxxx0.Motion_Control /*direction*/, 255 /*speed*/);
  }
}
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::ApplicationFunctionSet_Exploration(void)
...
void ApplicationFunctionSet::ApplicationFunctionSet_Exploration(void) {
  if (Application_OwlCarxxx0.Functional_Mode == Exploration_mode) {
    float getAnaloguexxx_R =
        AppITR20001
            .DeviceDriverSet_ITR20001_getAnaloguexxx_R(); // 获取传感器数据
    float getAnaloguexxx_L =
        AppITR20001.DeviceDriverSet_ITR20001_getAnaloguexxx_L();
    if ((getAnaloguexxx_R > TrackingDetection_CarLeaveTheGround) ||
        (getAnaloguexxx_L > TrackingDetection_CarLeaveTheGround)) {
      /*控制左右电机转动：后退*/
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::ApplicationFunctionSet_Tracking(void)
@ Functional function:  Owl Car 循迹
@ Input parameters:     none
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法
  模式进入：ApplicationFunctionSet_KeyCommand():1 /
ApplicationFunctionSet_SerialPortDataAnalysis()：N 101
...
void ApplicationFunctionSet::ApplicationFunctionSet_Tracking(void) {
  ...
  if (Application_OwlCarxxx0.Functional_Mode == TraceBased_mode) {
    ...
    AppSTM8S003F3_IR.DeviceDriverSet_STM8S003F3_IR_Get(
        &TrackingData_L /*uint16_t * STM8S003F3_IRL out*/,
    ...
    if (1 == function_xxx(TrackingData_M, TrackingDetection_S,
                          TrackingDetection_E)) // 前进 2==010
    {
      timestamp = true;
      BlindDetection = true;
      /*控制左右电机转动：实现匀速直行*/
    ...
    } else if (1 == function_xxx(TrackingData_L, TrackingDetection_S,
                                 TrackingDetection_E) &&
               // 0 == function_xxx(TrackingData_M, TrackingDetection_S,
               // TrackingDetection_E) &&
               0 == function_xxx(TrackingData_R, TrackingDetection_S,
                                 TrackingDetection_E)) // 左转 1==001 || 3==011
                                                       // 可以不关心 M
    {
      timestamp = true;
      BlindDetection = true;
    ...
    } else if (0 == function_xxx(TrackingData_L, TrackingDetection_S,
                                 TrackingDetection_E) &&
               // 0 == function_xxx(TrackingData_M, TrackingDetection_S,
               // TrackingDetection_E) &&
               1 == function_xxx(TrackingData_R, TrackingDetection_S,
                                 TrackingDetection_E)) // 右转 4==100 || 6==110
                                                       // 可以不关心 M
    {
      timestamp = true;
      BlindDetection = true;
    ...
    } else // 不在黑线上的时候。。。执行盲循迹   0==0000
    {
      ...
      if ((function_xxx((millis() - MotorRL_time), 0, 200) ||
           function_xxx((millis() - MotorRL_time), 600, 1000) ||
           function_xxx((millis() - MotorRL_time), 1500, 3600)) &&
          BlindDetection == true) {
      ...
      } else if (((function_xxx((millis() - MotorRL_time), 200, 600)) ||
                  function_xxx((millis() - MotorRL_time), 1000, 1400)) &&
                 BlindDetection == true) {
        if (1 == DirectionRecording) {
          ApplicationFunctionSet_OwlCarMotionControl(Left, 255);
        } else {
      ...
      } else if (((function_xxx((millis() - MotorRL_time), 1400, 1500))) &&
                 BlindDetection == true) {
        ApplicationFunctionSet_OwlCarMotionControl(Forward, 200);
      } else if ((function_xxx((millis() - MotorRL_time), 5000,
                               5500))) // Blind Detection ...s ?
      {
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::ApplicationFunctionSet_Obstacle(void)
@ Functional function:  Owl Car 避障
@ Input parameters:     none
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法
  模式进入：ApplicationFunctionSet_KeyCommand():2 /
ApplicationFunctionSet_SerialPortDataAnalysis()：N 101
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::CMD_MotorControl_xxx0(uint8_t is_MotorSelection, uint8_t
is_MotorDirection, uint8_t is_MotorSpeed)
...
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > CMD模式：N1指令运动模式 <电机控制> 接收并根据
APP端控制命令,执行对电机的单方向驱动(无时间限定：电机在运行时不接收时间的限定)
    *面向<电机>控制为对象
$ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
    ...
    // else
    {
      ...
      case 0: {
        ...
        if (1 == CMD_is_MotorDirection) { // 正转
          AppMotor.DeviceDriverSet_Motor_control(
              /*direction_A*/ direction_just, /*speed_A*/ is_MotorSpeed_A,
              /*direction_B*/ direction_just, /*speed_B*/ is_MotorSpeed_B,
              /*controlED*/ control_enable);     // Motor control
        } else if (2 == CMD_is_MotorDirection) { // 反转
          AppMotor.DeviceDriverSet_Motor_control(
              /*direction_A*/ direction_back, /*speed_A*/ is_MotorSpeed_A,
      ...
      case 1: {
        ...
        if (1 == CMD_is_MotorDirection) { // 正转
          AppMotor.DeviceDriverSet_Motor_control(
              /*direction_A*/ direction_void, /*speed_A*/ is_MotorSpeed_A,
              /*direction_B*/ direction_just, /*speed_B*/ is_MotorSpeed_B,
              /*controlED*/ control_enable);     // Motor control
        } else if (2 == CMD_is_MotorDirection) { // 反转
          AppMotor.DeviceDriverSet_Motor_control(
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::CMD_CarControlTimeLimit_xxx0(void)
@ Functional function:  Owl Car 车子控制
@ Input parameters:     uint8_t CMD_is_CarDirection,
1（左转）、2（右转）、3（前进）、4（后退） uint8_t CMD_is_CarSpeed, 电机速度
0-250 uint32_t CMD_is_Timer         执行时长
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > CMD模式：N2指令运动模式 <车子控制> 接收并根据
APP端控制命令,执行对车子的运动控制(有时间限定：电机在运行时接收时间限定)
      *面向<车子控制>为对象
$ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
void ApplicationFunctionSet::CMD_CarControlTimeLimit_xxx0(void) {
  // Application_FunctionSet.CMD_is_CarDirection,
  // Application_FunctionSet.CMD_is_CarSpeed,
  // Application_FunctionSet.CMD_is_CarTimer
  static boolean CarControl = false;
  static boolean CarControl_TE = false; // 还有时间标志
  static boolean CarControl_return = false;
  if (Application_OwlCarxxx0.Functional_Mode ==
      CMD_CarControl_TimeLimit) // 进入车子有时间限定控制模式
  {
    CarControl = true;
    if (CMD_is_CarTimer != 0) // #1设定时间不为..时 (空)
    {
      if ((millis() - Application_OwlCarxxx0.CMD_CarControl_Millis) >
          (CMD_is_CarTimer)) // 判断时间戳
      {
        CarControl_TE = true;
        ApplicationFunctionSet_OwlCarMotionControl(stop_it, 0);

        Application_OwlCarxxx0.Functional_Mode =
            CMD_Programming_mode; /*进入编程模式提示符<等待下一组控制命令的到来>*/
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:  void
ApplicationFunctionSet::CMD_CarControlNoTimeLimit_xxx0(void)
@ Functional function:  Owl Car 车子控制
@ Input parameters:     uint8_t CMD_is_CarDirection,
1（左转）、2（右转）、3（前进）、4（后退） uint8_t CMD_is_CarSpeed, 电机速度
0-250
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > CMD模式：N3指令运动模式 <车子控制> 接收并根据
APP端控制命令,执行对车子的运动控制(无时间限定：电机在运行时不接收时间限定)
      *面向<车子控制>为对象
$ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
void ApplicationFunctionSet::CMD_CarControlNoTimeLimit_xxx0(void) {
  ...
  static boolean CarControl = false;
  if (Application_OwlCarxxx0.Functional_Mode ==
      CMD_CarControl_NoTimeLimit) // 进入小车无时间限定控制模式
  {
    CarControl = true;
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:  void
ApplicationFunctionSet::CMD_MotorControlSpeed_xxx0(void)
@ Functional function:  Owl Car 电机控制
@ Input parameters:    uint8_t CMD_is_Speed_L(左电机的速度), uint8_t
CMD_is_Speed_R(右电机的速度)
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > CMD模式：N4指令运动模式 <电机控制> 接收并根据
APP端控制命令,执行对电机的运动控制(无时间限定：电机在运行时不接收时间限定)
      *面向<电机控制>为对象
$ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
void ApplicationFunctionSet::CMD_MotorControlSpeed_xxx0(void) {
  ...
  if (Application_OwlCarxxx0.Functional_Mode == CMD_MotorControl_Speed) {
    ...
    {
      AppMotor.DeviceDriverSet_Motor_control(
          /*direction_A*/ direction_just, /*speed_A*/ CMD_is_MotorSpeed_R,
...
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > CMD模式：N6指令声音模式 <声音控制> 接收并根据
APP端控制命令,执行对声音模块控制(有时间限定：声音在运行时接收时间限定)
    0#：时间结束后进入编程模式
      *面向<声音控制>为对象
$ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
void ApplicationFunctionSet::CMD_VoiceControl_xxx0(void) {
  if (Application_OwlCarxxx0.Functional_Mode == CMD_VoiceControl) {
    ApppassiveBuzzer.DeviceDriverSet_passiveBuzzer_controlAudio(
        CMD_is_controlAudio, CMD_is_VoiceTimer);
    Application_OwlCarxxx0.Functional_Mode =
        CMD_Programming_mode; /*进入编程模式提示符<等待下一组控制命令的到来>*/
#if _is_print
    Serial.print('{' + CommandSerialNumber + "_ok}");
...
static void CMD_Lighting(uint8_t is_LightingSequence,
                         int8_t is_LightingColorValue_R,
                         uint8_t is_LightingColorValue_G,
                         uint8_t is_LightingColorValue_B) {
  switch (is_LightingSequence) {
  case 0:
    AppRBG_LED.DeviceDriverSet_RBGLED_Color(NUM_LEDS, is_LightingColorValue_R,
                                            is_LightingColorValue_G,
                                            is_LightingColorValue_B);
    break;
  case 1: /*左*/
    AppRBG_LED.DeviceDriverSet_RBGLED_Color(3, is_LightingColorValue_R,
                                            is_LightingColorValue_G,
                                            is_LightingColorValue_B);
    break;
  case 2: /*前*/
    AppRBG_LED.DeviceDriverSet_RBGLED_Color(2, is_LightingColorValue_R,
                                            is_LightingColorValue_G,
                                            is_LightingColorValue_B);
    break;
  case 3: /*右*/
    AppRBG_LED.DeviceDriverSet_RBGLED_Color(1, is_LightingColorValue_R,
                                            is_LightingColorValue_G,
                                            is_LightingColorValue_B);
    break;
  case 4: /*后*/
    AppRBG_LED.DeviceDriverSet_RBGLED_Color(0, is_LightingColorValue_R,
                                            is_LightingColorValue_G,
                                            is_LightingColorValue_B);
    break;
  case 5: /*中*/
    AppRBG_LED.DeviceDriverSet_RBGLED_Color(4, is_LightingColorValue_R,
                                            is_LightingColorValue_G,
                                            is_LightingColorValue_B);
    break;
  default:
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::CMD_LightingControlTimeLimit_xxx0(void)
@ Functional function:  Owl Car RGB 显示
@ Input parameters:     CMD_is_LightingSequence (Left, front, right, back and
center) CMD_is_LightingColorValue_R, CMD_is_LightingColorValue_G,
                        CMD_is_LightingColorValue_B,
                        CMD_is_LightingTimer,
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > CMD模式：N7:指令 0# RGB Lighting Control with Time
Limitation 1# 进入编程模式 $ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
void ApplicationFunctionSet::CMD_LightingControlTimeLimit_xxx0(void) {
  ...
  if (Application_OwlCarxxx0.Functional_Mode ==
      CMD_LightingControl_TimeLimit) // 进入灯光有时间限定控制模式
  {
    LightingControl = true;
    if (CMD_is_LightingTimer != 0) // #1设定时间不为..时 (空)
    {
      if ((millis() - Application_OwlCarxxx0.CMD_LightingControl_Millis) >
          (CMD_is_LightingTimer)) // 判断时间戳
      {
    ...
    if (LightingControl_TE == false) {
      CMD_Lighting(CMD_is_LightingSequence, CMD_is_LightingColorValue_R,
                   CMD_is_LightingColorValue_G, CMD_is_LightingColorValue_B);
    }
  } else {
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::CMD_LightingControlNoTimeLimit_xxx0(void)
@ Functional function:  Owl Car RGB 显示
@ Input parameters:     CMD_is_LightingSequence (Left, front, right, back and
center) CMD_is_LightingColorValue_R, CMD_is_LightingColorValue_G,
                        CMD_is_LightingColorValue_B
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > CMD模式：N8:指令 0# RGB Lighting Control without Time
Limit 1# 进入编程模式 $ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
void ApplicationFunctionSet::CMD_LightingControlNoTimeLimit_xxx0(void) {
  static boolean LightingControl = false;
  if (Application_OwlCarxxx0.Functional_Mode ==
      CMD_LightingControl_NoTimeLimit) // 进入灯光无时间限定控制模式
  {
    LightingControl = true;
    CMD_Lighting(CMD_is_LightingSequence, CMD_is_LightingColorValue_R,
                 CMD_is_LightingColorValue_G, CMD_is_LightingColorValue_B);
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::CMD_LEDCustomExpressionControl_xxx0(void)
@ Functional function:  Owl Car LED 8*16矩阵板 表情显示
@ Input parameters:     none
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > CMD模式：N9:指令 0# LED matrix display expression 1#
进入编程模式 $ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
void ApplicationFunctionSet::CMD_LEDCustomExpressionControl_xxx0(void) {
  if (Application_OwlCarxxx0.Functional_Mode == CMD_ledExpressionControl) {
    Apptm1640.TM1640_FullScreenDisaple_led16x8(
        CMD_is_LEDCustomExpression_arry /*in*/);
    Application_OwlCarxxx0.Functional_Mode =
        CMD_Programming_mode; /*进入编程模式提示符<等待下一组控制命令的到来>*/
  }
}
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::CMD_LEDNumberDisplayControl_xxx0(void)
@ Functional function:  Owl Car LED 8*16矩阵板 数字显示
@ Input parameters:     CMD_is_LEDNumber （0--9）
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > CMD模式：N10:指令 0# LED matrix digital display 1#
进入编程模式 $ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
void ApplicationFunctionSet::CMD_LEDNumberDisplayControl_xxx0(void) {
  if (Application_OwlCarxxx0.Functional_Mode == CMD_ledNumberControl) {
    ...
    Apptm1640.TM1640_FullScreenDisaple_led16x8(
        Apptm1640.Display_Number[CMD_is_LEDNumber] /*in*/);
    Application_OwlCarxxx0.Functional_Mode =
        CMD_Programming_mode; /*进入编程模式提示符<等待下一组控制命令的到来>*/
  }
}
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::CMD_ClearAllFunctions_xxx0(void)
@ Functional function:  Owl Car 清除功能
@ Input parameters:     none
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > CMD模式：N100/N110:指令 0# N100
清除所有功能进入空闲模式 1# N110 清除所有功能进入编程模式 $ Elegoo & OwlCar Tank
Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
void ApplicationFunctionSet::CMD_ClearAllFunctions_xxx0(void) {
  if (Application_OwlCarxxx0.Functional_Mode ==
      CMD_ClearAllFunctions_Standby_mode) // 0# N100 清除所有功能进入空闲模式
  {
    ApplicationFunctionSet_OwlCarMotionControl(stop_it, 0);
    FastLED.clear(true);
    AppRBG_LED.DeviceDriverSet_RBGLED_xxx(0 /*Duration*/,
                                          5 /*Traversal_Number*/, CRGB::Black);
    Application_OwlCarxxx0.Motion_Control = stop_it;
    Application_OwlCarxxx0.Functional_Mode = Standby_mode;
  }
  if (Application_OwlCarxxx0.Functional_Mode ==
      CMD_ClearAllFunctions_Programming_mode) // 1# N110
                                              // 清除所有功能进入编程模式
  {
    ApplicationFunctionSet_OwlCarMotionControl(stop_it, 0);
    FastLED.clear(true);
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::CMD_UltrasoundModuleStatus_xxx0(uint8_t is_get)
@ Functional function:  Owl Car 向串口控制对象反馈循迹状态及数据
@ Input parameters:     is_get （0/1, 状态/数据）
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > CMD模式：N21:指令 0# 状态/数据 选择 1# 数据反馈 2#
切换到编程模式 The ultrasonic ranging and obstacle avoidance module receives and
feeds back the ultrasonic obstacle avoidance status and ranging data according
to the control command of APP terminal. $ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::CMD_TraceModuleStatus_xxx0(uint8_t is_get)
@ Functional function:  Owl Car 向串口控制对象反馈循迹状态及数据
@ Input parameters:     is_get （0/1, L/R）
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > CMD模式：N22:指令 0# L/R 选择 1# 数据反馈 2#
切换到编程模式 Tracking module receives and feeds back tracing status and data
according to the control command of APP terminal $ Elegoo & OwlCar Tank Kit &
2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
void ApplicationFunctionSet::CMD_TraceModuleStatus_xxx0(uint8_t is_get) {
  ...
  else if (3 == is_get) /*循迹状态获取右边*/
  {
    // 1# 数据反馈
    if (true == TrackingDetectionStatus_R ||
        true == TrackingDetectionStatus_M ||
        true == TrackingDetectionStatus_L) {
#if _is_print
      Serial.print('{' + CommandSerialNumber + "_true}");
#endif
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::CMD_TrajectoryControl_xxx0(void)
@ Functional function:  Owl Car 画线轨迹行径运动控制
@ Input parameters:     none
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > CMD模式：N103:指令 0# 获取运动坐标值的相对角度 1#
计算平面位置角度偏移误差值 2# 确定前后运动坐标的偏移方向 3# 直行运动 4#
切换到编程模式 5# 等待下一次运动坐标值更新 This part of the program is to deal
with APP and PC command control $ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
void ApplicationFunctionSet::CMD_TrajectoryControl_xxx0(void) {
  ...
  if (Application_OwlCarxxx0.Functional_Mode == CMD_TrajectoryControl) {
    ...
    {
      Trajectory_Record_X =
          CMD_is_TrajectoryControl_axisPlaneData_X; // 基坐标（更新虚拟圆点）
    ...
    // 0# 获取运动坐标值的相对角度
    getRngle(New_Record_X - Trajectory_Record_X /*in*/,
             New_Record_Y - Trajectory_Record_Y /*in*/,
             &Position_angle_E0 /*out*/);
    // 1 #计算平面位置角度偏移误差值
    Deviation_angle =
        Position_angle_E0 -
        Position_angle_E1; // 偏差角<（P-0） - （p-1）:本次 减 上次>
    ...
    if (NextCtrlCycleUp_flag == true) {
      // 2 #确定前后运动坐标的偏移方向
      if (LeftAngle <=
          360) // Position_angle_E1°
               // --------（+360°）逆时针：左边角度在控制范围内？<<<----
      {
        if (Position_angle_E0 > Position_angle_E1 /*本次的角度值要比上次的大*/
            && Position_angle_E0 <
                   LeftAngle /*本次的角度值要比可控范围小*/) { // 左前方运动
          DirectionControl = 1;
          ...
          if (Deviation_angle > 0) {
            ...
            Motion_angle = Deviation_angle;
                 ...
                 0) // Position_angle_E1°
                    // --------（+0°）顺时针：右边角度在控制范围内？---->>>
      {
        if (Position_angle_E0 < Position_angle_E1 &&
            Position_angle_E0 > RightAngle) { // 右前方运动
          DirectionControl = 2;
          Motion_angle = Deviation_angle;
        } else { // 左前方运动
          if (Deviation_angle < 0) {
      ...
      uint16_t Motion_angle_time =
          (750.0 / 90.0) *
          Motion_angle; // 通过控制电机运动时间来改变偏移的位置角度
      ...
      if (2 == DirectionControl) // 前左方向运动控制
      {
        ...
        //   ApplicationFunctionSet_OwlCarMotionControl(Left /*in:direction*/,
        //   255 /*in:0 < speed > 250*/);
      ...
      } else if (1 == DirectionControl) // 前右方向运动控制
      {
        ...
        //   wdt_reset();
        //   AppMPU6050getdata.DeviceDriverSet_MPU6050_dveGetEulerAngles(&_yaw);
        //   ApplicationFunctionSet_OwlCarMotionControl(Right /*in:direction*/,
        //   255 /*in:0 < speed > 250*/);
        // } while ((yaw_So + Motion_angle) > _yaw);
        ApplicationFunctionSet_OwlCarMotionControl(Right /*in:direction*/,
                                                   255 /*in:0 < speed > 250*/);
      ...
      // 3# 直行运动
      ApplicationFunctionSet_OwlCarMotionControl(Forward /*in:direction*/,
                                                 255 /*in:0 < speed > 250*/);
    }
    // 4# 切换到编程模式
    Application_OwlCarxxx0.Functional_Mode = CMD_Programming_mode;
...
/*
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
@ Function prototype:   void
ApplicationFunctionSet::ApplicationFunctionSet_KeyCommand(void)
@ Functional function:  Owl Car 模式按键值获取
@ Input parameters:     none
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > $ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
...
@ Output parameters:    none
@ Other Notes:          此为ApplicationFunctionSet层公共方法 <调用硬件驱动层:
DeviceDriverSet_xxx0_H > 1#接收串口数据流 2#导入JsonDocument
    3#解析并更新控制命令的信号量值
$ Elegoo & OwlCar Tank Kit & 2019-09
 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
...
void ApplicationFunctionSet::ApplicationFunctionSet_SerialPortDataAnalysis(
  ...
  if (c == '}') {
    ...
    DeserializationError error =
        deserializeJson(doc, SerialPortData); // 反序列化JSON数据
    SerialPortData = "";
    if (!error) // 检查反序列化是否成功
    {
      ...
      case 7: /*<命令：N 7> */
        Application_OwlCarxxx0.Functional_Mode =
            CMD_LightingControl_TimeLimit; /*灯光控制:有时间限定模式*/
        CMD_is_LightingSequence =
            doc["D1"]; // Lighting (Left, front, right, back and center)
      ...
      case 8: /*<命令：N 8> */
        Application_OwlCarxxx0.Functional_Mode =
            CMD_LightingControl_NoTimeLimit; /*灯光控制*/
        CMD_is_LightingSequence =
            doc["D1"]; // Lighting (Left, front, right, back and center)
        CMD_is_LightingColorValue_R = doc["D2"];
        CMD_is_LightingColorValue_G = doc["D3"];
      ...
      case 9: /*<命令：N 9> */
        Application_OwlCarxxx0.Functional_Mode =
            CMD_ledExpressionControl; /*led矩阵显示表情控制*/
...
#endif
        {
          ...
          for (int is_true_j = 0; is_true_j < 16; is_true_j++) {
            for (int is_true_i = 0; is_true_i < 8; is_true_i++) {
              if ((is_arry[is_true_i] << is_true_j) &
                  0X8000) // 取双字节最高位 如果为 1?
              {
            ...
            CMD_is_LEDCustomExpression_arry[is_true_j] =
                is_byte; // 更新单字节数据缓存
            is_byte = 0x00;
          }
        }
      ...
      case 10: /*<命令：N 10> */
        Application_OwlCarxxx0.Functional_Mode =
            CMD_ledNumberControl; /*led矩阵显示数字控制*/
      ...
      case 110: /*<命令：N 110> */
        Application_OwlCarxxx0.Functional_Mode =
            CMD_ClearAllFunctions_Programming_mode; /*清除功能:进入编程模式*/
        break;
      case 100: /*<命令：N 100> */
        Application_OwlCarxxx0.Functional_Mode =
            CMD_ClearAllFunctions_Standby_mode; /*清除功能：进入空闲模式*/
        break;
      ...
      case 102: /*<命令：N 102> :摇杆控制命令*/
        ...
        // ApplicationFunctionSet_OwlCarMotionControl(Application_OwlCarxxx0.Motion_Control
        // /*direction*/, 255 /*speed*/);
#if _is_print
        // Serial.print('{' + CommandSerialNumber + "_ok}");
#endif
