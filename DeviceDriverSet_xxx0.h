...
// #define TimeCompensation 4 //时间补偿系数  （由于修改了T0定时器，导致标准库的
// delay() 、millis()函数出现偏差，特此设置误差调节） extern unsigned long
// millis(); extern void _delay(unsigned long ms);
/*RBG LED*/
#include "FastLED.h"
class DeviceDriverSet_RBGLED {
public:
  void DeviceDriverSet_RBGLED_Init(uint8_t set_Brightness);
  void DeviceDriverSet_RBGLED_xxx(uint16_t Duration, uint8_t Traversal_Number,
                                  CRGB colour);
#if _Test_DeviceDriverSet
  void DeviceDriverSet_RBGLED_Test(void);
#endif
  void DeviceDriverSet_RBGLED_Color(uint8_t LED_s, uint8_t r, uint8_t g,
                                    uint8_t b);
...
public:
  void
  DeviceDriverSet_passiveBuzzer_controlMonosyllabic(uint8_t controlMonosyllabic,
                                                    uint32_t Duration);
  void DeviceDriverSet_passiveBuzzer_controlAudio(uint16_t controlAudio,
                                                  uint32_t Duration);
...
#define PIN_passiveBuzzer 5
  ...
  int melody[8] = {NOTE_C5, NOTE_D5, NOTE_E5, NOTE_F5,
                   NOTE_G5, NOTE_A5, NOTE_B5, NOTE_C6}; // notes in the melody
  // int melody[8] = {523, 587, 659, 698, 784, 880, 988, 1047}; // notes in the
  // melody
};
/*Key Detection*/
...
public:
  ...
  void DeviceDriverSet_Motor_control(boolean direction_A,
                                     uint8_t speed_A, // A组电机参数
                                     boolean direction_B,
                                     uint8_t speed_B,  // B组电机参数
...
#endif
  void DeviceDriverSet_STM8S003F3_IR_Get(uint16_t *STM8S003F3_IRL /*out*/,
                                         uint16_t *STM8S003F3_IRM /*out*/,
...
public:
  ...
  void DeviceDriverSet_Servo_Sweep(void);
